...
export function BacktestingTrades({ result }: BacktestingTradesProps) {
  // 过滤出已关闭的交易
  const closedTrades = result.trades.filter((trade) => !trade.signal);
        ...
        <div className="divide-y">
          {closedTrades.map((trade, index) => (
            <div key={index} className="grid grid-cols-6 p-3">
              ...
              >
                {trade.signal?.type === "buy" ? "买入" : "卖出"}
              </div>
              <div className="text-sm">{trade.kline.date}</div>
              <div className="text-sm">${trade.signal?.price.toFixed(2)}</div>
              <div className="text-sm">{trade.signal?.quantity}</div>
